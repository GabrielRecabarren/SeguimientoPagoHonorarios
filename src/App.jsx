...
const appId = import.meta.env.VITE_APP_ID || 'honorarios-tracker-v1';

let app, auth, db;
const firebaseConfigured = firebaseConfig.projectId && firebaseConfig.apiKey;
...
export default function App() {
  ...
  const [firebaseError, setFirebaseError] = useState(!firebaseConfigured);
  ...
  const updateStatus = async (prestador, mes, estado) => {
    if (!firebaseConfigured) {
      setData(prev => ({
        ...prev,
        [prestador]: { ...(prev[prestador] || {}), [mes]: estado }
      }));
      return;
    }
    if (!user) return;
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 p-4 md:p-8 font-sans">
      {firebaseError && (
        <div className="max-w-7xl mx-auto mb-4 p-4 bg-yellow-50 border border-yellow-300 rounded-lg flex items-center gap-2 text-yellow-800 text-sm">
          <AlertCircle className="w-4 h-4 flex-shrink-0" />
          <span>Firebase no configurado. Los cambios se guardan localmente (sin sincronización). Configure las variables de entorno VITE_FIREBASE_* para habilitar la sincronización.</span>
        </div>
      )}
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-gray-500 text-sm">
                {firebaseConfigured ? 'Uso colaborativo habilitado' : 'Modo local (sin sincronización)'}
              </p>
            </div>
          </div>
